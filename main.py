...
class Header(BoxLayout):
    ...
    def build_ui(self):
        ...
        self.status_indicator.bind(on_ref_press=lambda x: None)
        ...
        # Hamburger menu button (using Button for better touch support)
        self.hamburger_btn = Button(
            text="☰",
            font_size=sp(20),
            bold=True,
            color=(0.88, 0.9, 0.93, 1),  # #e0e6ed
            background_color=(0, 0, 0, 0),
            size_hint=(None, 1),
            width=dp(40),
            on_press=self.app.open_hamburger_menu
        )
...
class HamburgerMenu(Popup):
    ...
    def build_ui(self):
        content = BoxLayout(orientation='vertical', spacing=dp(4), padding=dp(12))
        
        # Menu items
        menu_items = [
            ("🔗 Connect", self.app.switch_to_connect),
            ("🌐 View", self.app.switch_to_view),
            ("👤 Account", self.app.switch_to_account),
        ...
        for item in menu_items:
            ...
            btn = Button(
                ...
                size_hint=(1, None),
                height=dp(40),
                background_color=(0.15, 0.15, 0.2, 1) if enabled else (0.1, 0.1, 0.15, 1),
                color=(0.88, 0.9, 0.93, 1) if enabled else (0.4, 0.45, 0.5, 1),
        ...
        # Auto-connect toggle
        auto_layout = BoxLayout(orientation='horizontal', size_hint=(1, None), height=dp(36))
        ...
        # Full screen toggle (mobile uses different approach)
...
class MzindaTrackApp(App):
    ...
    def update_auto_indicator(self):
        """Update auto-connect indicator in header"""
        if self.auto_connect_enabled:
            self.header.auto_status_label.text = "🔄"
            self.header.auto_status_label.opacity = 1
            self.header.auto_status_label.color = (0.39, 1, 0.85, 1)
        else:
            self.header.auto_status_label.text = ""
    ...
    def toggle_fullscreen(self, instance=None):
        ...
        if self.is_fullscreen:
            # Enter fullscreen - hide header and status bar
...
class ConnectScreen(Screen):
    ...
    def build_ui(self):
        ...
        content = BoxLayout(orientation='vertical', spacing=spacing, size_hint_y=None)
        ...
        # Server URL
        server_group = BoxLayout(orientation='vertical', spacing=dp(8), size_hint_y=None, height=dp(150))
        server_label = Label(
            text="Server URL",
            font_size=sp(14),
            bold=True,
    ...
    def test_server(self, instance):
        ...
        def on_result(result):
            ...
            if result.get('status') == 'healthy':
                ...
                self.app.header.status_indicator.text = "✅"
...
class ViewScreen(Screen):
    ...
    def build_ui(self):
        ...
        # Simple web view placeholder with toolbar
